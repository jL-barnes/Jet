...
void make_nickel( struct domain * theDomain ){
   ...
   if( NUM_N>0 ){
      ...
      for( jk=0 ; jk<Nt*Np ; ++jk ){
	    ...
	    // Also keep track of Nickel produced using the old method
	    // The threshold value of Pp is dependent on the ejecta model properties.
	    // It is set through the parameter list (in.par)
	    if( Pp > Pth ){
	      if( NUM_N > 2 ){
		 c->prim[NUM_C+2] = 1.0; 
		 c->cons[NUM_C+2] = c->cons[DEN]; 
	      }
	    }
	 }
      }    
